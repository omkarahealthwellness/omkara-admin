...
export default function NavigationPage() {
  ...
  const { data: nav, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return snap.exists() ? (snap.data() as NavigationData) : {
        items: [
          { id: crypto.randomUUID(), label: "Home", url: "https://example.com", icon: "HOME" as NavIcon, visible: true, sortOrder: 0 },
          { id: crypto.randomUUID(), label: "Products", url: "https://example.com/products", icon: "MENU" as NavIcon, visible: true, sortOrder: 1 },
        ]
      };
    },
  });

  const { register, control, handleSubmit, formState: { errors } } = useForm<NavigationData>({
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {fields.map((field, index) => (
              <div key={field.id} className="flex items-center gap-3 p-3 bg-muted/30 border rounded-md group">
                <button 
                  type="button" 
                ...
                <div className="flex-1 grid grid-cols-12 gap-3">
                  <div className="col-span-4">
                    ...
                    />
                    {errors.items?.[index]?.label && <p className="text-xs text-destructive mt-1">{errors.items[index]?.label?.message}</p>}
                  </div>
                  <div className="col-span-5">
                    <Input 
                      ...
                      className="bg-background"
                    />
                    {errors.items?.[index]?.url && <p className="text-xs text-destructive mt-1">{errors.items[index]?.url?.message}</p>}
                  </div>
            ...
            <Button 
              ...
              onClick={() => append({ id: crypto.randomUUID(), label: "", url: "https://", icon: "HOME", visible: true, sortOrder: fields.length })}
            >
