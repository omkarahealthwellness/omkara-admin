  ...
  try {
    ...
    try {
      ...
      if (env && env.MANIFEST_KV) {
        ...
        const logEntry = JSON.stringify({ timestamp: new Date().toISOString(), version: versionId, user: "admin", hash: hashHex });
    ...
    return NextResponse.json({
      ...
      message: kvSuccess ? "Published to KV successfully!" : "Manifest assembled, but KV upload skipped (local mode)."
    });
